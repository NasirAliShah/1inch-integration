/**
 * 1inch API Base Network Single Address Swap
 * Purpose: Get swap transaction data for ETH to USDC on Base network
 * 
 * This generates the actual transaction data that can be signed and executed
 * Similar to getSwapTransaction.js but for Base network
 */
...
// Swap Configuration
const AMOUNT_IN_WEI = '100000000000000'; // 0.0001 ETH (very small amount for testing)
const WALLET_ADDRESS = '0x3639e6f4c224ebd1bf6373c3d97917d33e0492bb'; // Address 2 (has ETH balance)
const SLIPPAGE_PERCENT = '1'; // 1% slippage tolerance

/**
 * Fetches swap transaction data from 1inch API on Base network
...
async function getSwapTransaction(fromToken, toToken, amount, fromAddress, slippage) {
  try {
    ...
    const params = new URLSearchParams({
      ...
      dst: toToken,             // Destination token (USDC)
 ...
 */
async function getQuote(fromToken, toToken, amount, fromAddress) {
  try {
    if (!API_KEY) {
...
function displayQuote(quoteData) {
  ...
  const fromDecimals = quoteData.fromToken?.decimals || 18;
  const toDecimals = quoteData.toToken?.decimals || 6; // USDC has 6 decimals
  const fromSymbol = quoteData.fromToken?.symbol || 'ETH';
...
async function main() {
  try {
    console.log('🚀 Starting 1inch Base Network Swap Transaction Generation...\n');
    console.log(`🌐 Network: Base (Chain ID: ${CHAIN_ID})`);
    ...
    // Step 2: Get swap transaction data for ETH -> USDC swap
